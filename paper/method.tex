...
\begin{equation}\label{eq:pos_recovery}
    \hat{u}_i(t+1) = u_i(t) + \Delta\hat{u}_i(t), \qquad
...
\begin{equation}
    \Psi = \tfrac{1}{2}\lambda(\log|\det\mathbf{F}|)^2 - \mu\log|\det\mathbf{F}| + \tfrac{1}{2}\mu\big(\mathrm{tr}(\mathbf{F}^\top\mathbf{F}) - 3\big),
\end{equation}
