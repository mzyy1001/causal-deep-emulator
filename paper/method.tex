...
Stage~2 runs for 40 epochs with the Adam optimiser (initial learning rate $10^{-5}$, decay factor 0.98, batch size 4). During Stage~2, the model trains on both sphere geometry data and character motion sequences (self-supervised, no ground-truth displacements required), enabling generalization to unseen mesh topologies.
...
We train on sphere primitives (7 sequences, stiffness $5\!\times\!10^4$--$5\!\times\!10^6$) and evaluate generalization to 5 characters (michelle, big\_vegas, kaya, mousey, ortiz) across 10 motions. Ground-truth displacements for stiffness-robustness evaluation are generated via FEM simulation at 6 stiffness values ($10^4$--$5\!\times\!10^6$), including extrapolation below the training range.

We compare against the original Deep Emulator~\cite{zheng2021deep} as baseline, and an ablation of our model with the causal cone disabled (uniform mask).

\subsection{Stiffness Generalization on Characters}

Table~\ref{tab:fem_main} shows single-step displacement MSE against FEM ground truth on character meshes at varied stiffness. Our model (with Stage~2 physics fine-tuning) achieves $2.2\times$ lower error than the baseline on mousey, the most deformable character (1066 free vertices, 46\% of mesh).

\begin{table}[h]
\centering
\caption{Single-step MSE vs FEM ground truth on characters at varied stiffness. \textbf{Bold} = best.}
\label{tab:fem_main}
\small
\begin{tabular}{rc|cc|cc}
\toprule
& & \multicolumn{2}{c|}{\textbf{mousey/dancing\_1}} & \multicolumn{2}{c}{\textbf{michelle/cross\_jumps}} \\
\textbf{Stiffness} & \textbf{In Train?} & \textbf{Ours} & \textbf{Baseline} & \textbf{Ours} & \textbf{Baseline} \\
\midrule
10,000 & NO & \textbf{0.0231} & 0.0500 & \textbf{0.0459} & 0.0469 \\
50,000 & YES & \textbf{0.0233} & 0.0502 & \textbf{0.0458} & 0.0464 \\
100,000 & YES & \textbf{0.0233} & 0.0503 & \textbf{0.0458} & 0.0464 \\
500,000 & YES & \textbf{0.0233} & 0.0507 & \textbf{0.0457} & 0.0464 \\
1,000,000 & YES & \textbf{0.0233} & 0.0512 & \textbf{0.0458} & 0.0465 \\
5,000,000 & YES & \textbf{0.0233} & 0.0555 & \textbf{0.0457} & 0.0465 \\
\midrule
\textbf{Avg} & & \textbf{0.0233} & 0.0513 & \textbf{0.0458} & 0.0465 \\
\bottomrule
\end{tabular}
\end{table}

All models exhibit excellent stiffness robustness: MSE varies less than 3\% across a $500\times$ stiffness range, including extrapolation to the unseen value $10^4$.

\subsection{Causal Cone Ablation}

Disabling the causal cone (replacing the learned mask with a uniform mask) increases average character MSE by $66\%$ (from $0.00845$ to $0.02501$), confirming that the material-conditioned spatiotemporal gating is the key contributor to generalization.

\subsection{Resource Usage}

Our model (761K parameters) runs at 20.4 FPS on an NVIDIA A10, using 290~MB peak GPU memory. The baseline (237K parameters) achieves 687~FPS at 31~MB. The overhead is dominated by multi-scale edge aggregation, not the causal cone itself (which adds $<0.1$~ms per frame).
